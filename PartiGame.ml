module PartiGame :
  sig
		type pg_graph
		val step_execute : pg_graph -> bool
		val graph_from_image : string -> int array -> int -> pg_graph
		val display_initial_grid : pg_graph -> unit
		val split_of_clic : pg_graph -> unit
		val run_partigame : pg_graph -> int -> unit
  end =
  struct
		(* Global flags *)
		let h_eps = 1.;;
